...
class ScrabbleServerFactory(protocol.ServerFactory, object):
    ...
    def addNewUser(self, username, password, isAdmin):
        ...
        if 2 > len(username) > constants.MAX_NAME_LENGTH:
    ...
    def authenticate(self, username, password):
        ...
        if self.db.users.has_key(u):
            ...
            return self.addNewUser(username,password,False)[0]
    ...
    def loginUser(self, player, client):
        ...
        try:
            ...
            print "something is very wrong, key error"
            pass
        self.db.sync()
    ...
    def gameSendMove(self, gameId, onboard, moves, client):
        ...
        for move in moves:
            ...
            if word not in self.dicts[ game.options[OPTION_RULES] ]:
                ...
                return
            if game.options[OPTION_NO_REPEATS]:
                if word in game.words:
                    client.gameError(gameId, ServerMessage([word, ALREADY_ON_BOARD]))
                    return
            words.append( word )
        ...
        if (len(player.getLetters()) == 0 and game.isBagEmpty()):
            ...
            for p in players:
                ...
                for letter in letters:
                    ...
                    if game.options[OPTION_FINAL_LETTER]:
